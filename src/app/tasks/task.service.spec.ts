...
describe('CustomersService', () => {
  ...
  it('should return an expected list of customers', (done: DoneFn) => {
    ...
    expect(httpSpy.get).toHaveBeenCalledOnceWith('/api/tasks',
  ...
  it('#addTaskCall should add task', (done: DoneFn) => {
    ...
    expect(httpSpy.post).toHaveBeenCalledOnceWith('/api/addTask', expectedTask,
      Object({ withCredentials: true }));
  });
  ...
  it('#updateTaskCall should update task', (done: DoneFn) => {
    ...
    expect(httpSpy.post).toHaveBeenCalledOnceWith('/api/updateTask', expectedTask,
      Object({withCredentials: true}));
  });
  ...
  it('#deleteTaskCall should delete task', (done: DoneFn) => {
    ...
    expect(httpSpy.delete).toHaveBeenCalledOnceWith('/api/task?id=' + taskId,
  ...
  it('#makeTaskDoneCall should make task done', (done: DoneFn) => {
    ...
    expect(httpSpy.post).toHaveBeenCalledOnceWith('/api/makeTaskDone?id=' + taskId,
      {withCredentials: true});
  });
  ...
  it('#makeTaskToDoCall should make task todo', (done: DoneFn) => {
    ...
    expect(httpSpy.post).toHaveBeenCalledOnceWith('/api/makeTaskToDo?id=' + taskId,
      {withCredentials: true});
  });
