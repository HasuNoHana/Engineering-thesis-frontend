...
export class DevPageComponent {
  ...
  undoneTask() {
    this.http.get<string>('http://localhost:4200/api/dev/undoneTask', {withCredentials: true})
  ...
  getTaxes() {
    this.http.get<string>('http://localhost:4200/api/dev/getTaxes', {withCredentials: true})
      .subscribe((answer: string) => {
        console.log("answer")
        debugLogOnlyMessage("dupa")
        this.houseService.fetchData();
